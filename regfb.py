import subprocess, time, random, os, unidecode, requests, sys, cv2, numpy as np, requests, re
import data_run
os.system('cls')
...
def creat_acc():
    ...
    time.sleep(1)
    click('logo.png', 3, 'Đang mở app facebook lite')
    click('tiengviet.png', 3, 'Chuyển đổi ngôn ngữ sang tiếng việt')
    click('taotaikhoanmoi.png', 2, 'Tạo mới tài khoản')
    click('tiep.png', 1, 'Nhấn tiếp')
    putdata('x', 0, '')
    system_cmd('KEYCODE_DEL')
    putdata(last_name, 5, 'Nhập Họ')
    click('ten.png', 1, 'Nhấn sang ô nhập tên')
    putdata(fist_name, 5, 'Nhập tên')
    click('tiep.png', 2, 'Nhấn tiếp')
    click('dangkybangmail.png', 1, 'Nhấn đăng ký bằng gmail')
    click('nhapmail.png', 1, 'Nhấn ô nhập mail')
    email = get_mail(1)
    ...
    click('tiep.png', 2, 'Nhấn next')
    click('dangky.png', 20, 'Xác nhận tạo tài khoản')
    click('luckhac.png', 2, 'Nhan luc khac')
